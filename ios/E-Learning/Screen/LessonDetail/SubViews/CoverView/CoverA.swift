...
//  E-Learning
//
//  Created by Nguyen Van Nhat on 1/16/18.
...
class CoverA: UIView, MMPlayerCoverViewProtocol {
    ...
    @IBAction func btnAction() {
        ...
        if playAgain {
            playAgain = false
            playLayer?.autoPlay = true
            delegate?.playAgain()
        } else if playLayer?.player?.rate == 0 {
    ...
    func currentPlayer(status: MMPlayerPlayStatus) {
        switch status {
        case .playing:
            self.btnPlay.setImage(UIImage(named: "ic_pause_circle_filled")?.withRenderingMode(.alwaysTemplate), for: .normal)
        default:
            self.btnPlay.setImage(UIImage(named: "ic_play_circle_filled")?.withRenderingMode(.alwaysTemplate), for: .normal)
        }
    }
    ...
    func timerObserver(time: CMTime) {
        if let duration = self.playLayer?.player?.currentItem?.asset.duration ,
            ...
            self.labTotal.text = self.convert(second: duration.seconds)
            self.playSlider.value = Float(time.seconds)
            let timeLeft = duration.seconds - time.seconds
            if timeLeft <= 0 {
                self.btnPlay.setImage(UIImage(named: "ic_play_circle_filled")?.withRenderingMode(.alwaysTemplate), for: .normal)
                playAgain = true
            }
